/* Student Name: Daniele del Pozzo */
/* N number: 12345678 */
...
INSERT INTO `meal` VALUES (7,'Spaghetti Bolognaise', 'lunch');
INSERT INTO `meal` VALUES (8,'Lasagne', 'lunch');
INSERT INTO `meal` VALUES (9,'Fettuccine Alfredo', 'lunch');
INSERT INTO `meal` VALUES (10,'Pasta Carbonara', 'lunch');
INSERT INTO `meal` VALUES (11,'Ravioli', 'lunch');
INSERT INTO `meal` VALUES (12,'Macaroni Cheese', 'lunch');
INSERT INTO `meal` VALUES (13,'Pasta Cacio e Pepe', 'lunch');
INSERT INTO `meal` VALUES (14,'Spaghetti alle Vongole', 'lunch');
INSERT INTO `meal` VALUES (15,'Anticucho', 'lunch');
INSERT INTO `meal` VALUES (16,'Asocena', 'lunch');
INSERT INTO `meal` VALUES (17,'Barbacoa', 'lunch');
INSERT INTO `meal` VALUES (18,'Tofu Lettuce Wraps', 'lunch');
INSERT INTO `meal` VALUES (19,'Braised Chickpeas with Chard', 'lunch');
INSERT INTO `meal` VALUES (20,'Tofu Stir-Fry with Peanut Sauce', 'lunch');
INSERT INTO `meal` VALUES (21,'Vegetarian Meatballs', 'lunch');
...
(SELECT SSN, Name
from employee
WHERE employee.CivilStatus = 'single' AND employee.Name LIKE 'A%' AND employee.Name IS NOT NULL);
...
FROM adult
JOIN guest ON adult.adult_ID = guest.ID
WHERE guest.accompany_ID IS NULL
...
AS
(SELECT guest.ID AS ID, guest.LName AS LName, room.Number AS RoomNumber
FROM guest
JOIN booking ON guest.ID = booking.guest_ID
JOIN room ON booking.room_ID = room.ID
WHERE booking.To < '2022-03-01');
  

-- /* (e) CREATE TABLE ANSWER05 as */
CREATE TABLE ANSWER05
AS
(SELECT employee.SSN AS SSN, employee.Name AS Name, Speciality AS Specialty, meal.Title AS MealTitle
FROM cook
JOIN employee ON cook.cook_ID = employee.ID
JOIN orders ON cook.cook_ID = orders.cook_ID
JOIN meal ON orders.meal_ID = meal.ID
...
AS
(SELECT guest.ID, guest.FName, guest.LName, SUM(booking.Price) AS Sum_of_prices
FROM guest
JOIN booking ON guest.ID = booking.guest_ID
JOIN room ON booking.room_ID = room.ID
JOIN double_room ON room.ID = double_room.double_ID
WHERE double_room.BedType = 'twin'
GROUP BY guest.ID, guest.FName, guest.LName
...
JOIN booking booking2 ON booking1.ID != booking2.ID
WHERE booking1.from > booking2.from
AND booking1.ID < booking2.ID);

-- /* (h) CREATE TABLE ANSWER08 as */
CREATE TABLE ANSWER08
AS
(SELECT DISTINCT receptionist.receptionist_ID AS ID
FROM receptionist
JOIN booking ON receptionist.receptionist_ID = booking.receptionist_ID
JOIN orders ON booking.guest_ID = orders.guest_ID
WHERE orders.meal_ID = 3
ORDER BY receptionist.receptionist_ID ASC);
  

-- /* (i) CREATE TABLE ANSWER09 as */
CREATE TABLE ANSWER09
AS
(SELECT guest.accompany_ID AS ID, guest.FName AS FName, guest.LName AS LName
FROM guest
JOIN adult ON guest.ID = adult.adult_ID
WHERE adult.Age = (SELECT MAX(Age) FROM adult)
);
...
AS
(SELECT clean.housekeeper_ID AS ID, COUNT(clean.ID) AS Number_of_cleanings
FROM clean
JOIN room ON clean.room_ID = room.ID
WHERE room.View = 'lake view' AND clean.Date < '2022-04-01'
GROUP BY clean.housekeeper_ID
HAVING COUNT(clean.ID) >= 3);
